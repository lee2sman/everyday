...
let sleepSnd;
function preload() {
	dustImg = loadImage("assets/dust.png");
	sleepSnd = loadSound("assets/sleep.mp3");
	for (let i = 0; i < 31; i++) {
...
angleMode(DEGREES)
	for (let i = 0; i < 5; i++) {
		people[i] = new Face(random(width), random(height));
	}
}
function mousePressed() {
	if (!sleepSnd.isPlaying()) {
		sleepSnd.loop();
	}
}
...
function draw() {
  ...
  rotate(rot)
	image(dustImg, 0,0, width, height);
...
class Face {
	constructor(_x, _y) {
		...
		this.img = random(faceImg);
		if (random() < 0.1) {
			this.box = createCheckbox(random(traits), true);
		} else {
			this.box = createCheckbox(random(traits), false);
		}
	}
	display() {
		let x = this.x * noise(frameCount * 0.003);
		let y = this.y * noise(frameCount * 0.004);
		image(this.img, x, y);
		this.box.position(x + 150, y);
