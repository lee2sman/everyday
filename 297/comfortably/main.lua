...
function setup()
  ...
  people={}
  for i=0,5 do
    table.insert(people, Face:new(random(width),random(height)))
  end
end
...
function Face:new(x, y, w, h)
  local face = {
    x = x or random(width), 
    y = y or random(height), 
    img = faceImg[floor(random(#faceImg))+1],
    --img = random(faceImg),
    msg = random(traits)
  }
  setmetatable(face, Face) 
  return face
end

function Face:display()
  local x = self.x * noise(frameCount * 0.001)
  local y = self.y * noise(frameCount * 0.002)

  image(self.img,x,y)
  text(self.msg,x+150,y)

  if random(1000)<1 then
    self.msg = random(traits)
  end
end
