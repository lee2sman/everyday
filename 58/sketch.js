// self playing mountain system number none props 17
//remix 57 to 58
...
let title, titleWords = ["birth","introduction","reveal","leaving","exiting","embarking",
  "parting","delivery","donation","presentation","endowment","reveal","exposure"
	,"separation","death","abandonment","awakening","rise","improvement",
...
let textX = 30, f, f2, roomText, fortune = [
...
function setup() {
  ...
  rectMode(CENTER);
	title = titleWords[floor(random(titleWords.length))] + ' && ' + titleWords[floor(random(titleWords.length))];
...
function resetRoom(){
  ...
  questNum++;
	...
	while ((bird.velocity.x > -1) && (bird.velocity.x < 1) && (bird.velocity.y > -1) && (bird.velocity.y < 1)){
    bird.velocity.x = random(-5,5);
    bird.velocity.y = random(-5,5);
	}
