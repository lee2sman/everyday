let canvas;
let textureImgs = [];
let customers = [], totalCustomers = 20;
let fonttype;
let current
...
setup = () => {
	...
	for (let i = 0; i < totalCustomers; i++) {
		customers[i] = new Customer(random(-width/2,width/2),random(-height/2,height/2));
	}
...
draw = () => {
  //background
  image(textureImgs[2],-width/2,-height/2,width,height)
  orbitControl();
  showCustomer()
};
...
mousePressed = () => {
  for (let i = 0; i < totalCustomers;i++){
       customers[i].offsetX = random(-width/2,width/2)
       customers[i].offsetY = random(-height/2,height/2)
  }
};
...
class Customer {
	constructor(tempOffsetX,tempOffsetY) {
		this.name = getName();
		this.place = getName();
		this.item = getObject();
	...
	talk() {
		...
		fill(0);
	  push()
	  translate(this.offsetX,this.offsetY+150)
		text(this.name + " of " + this.place + " desires a " + this.item, 0, 0,325,150);
	  pop()
	}
	shopkeeper = () => {
		//loc
